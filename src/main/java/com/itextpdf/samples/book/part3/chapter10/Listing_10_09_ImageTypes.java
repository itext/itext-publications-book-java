...
public class Listing_10_09_ImageTypes {
    ...
    public static final String RESOURCE = "src/main/resources/img/hitchcock.png";
    ...
    public void manipulatePdf(String dest) throws IOException {
        ...
        for (int i = 0; i < RESOURCES.length; i++) {
            if (String.format("src/main/resources/img/%s", RESOURCES[i]).contains(".wmf")) {
                WmfImageHelper wmf
                        = new WmfImageHelper(new WmfImageData(String.format("src/main/resources/img/%s", RESOURCES[i])));
                img = new Image((PdfFormXObject) wmf.createFormXObject(pdfDoc));
            } else {
                ImageData image
                        = ImageDataFactory.create(String.format("src/main/resources/img/%s", RESOURCES[i]));
